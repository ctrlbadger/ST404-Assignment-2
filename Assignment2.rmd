...
We perform forward, backward and hybrid stepwise regression according to both AIC and BIC. We see that the models generated for AIC are all the same and the ones generated for BIC are all the same. In the AIC case we have the model has 12 parameters, whereas the BIC model has 11 variables. (See Appendix 2.2.1)
...
# Below we perform stepwise regression for both AIC
# We alsp performed similar regression for BIC
cancermodel <- cancer %>% select(
  !c("Geography", "medIncome", "binnedInc", "PctBlack"))
c0=lm(deathRate~1,cancermodel)
cmax=lm(deathRate~.,cancermodel)
...
```{r stepwise plots A,eval=FALSE,fig.asp=0.5, fig.cap="Residuals plot of Hybrid Stepwise Model using BIC"}
# An example code for residuals plot
plot(hybridoptimalBIC,which=1)
```
...
```{r statestepwise 4 A, include=FALSE}
# Perform F-Test on the BIC models above
anova(hybridoptimalBIC,hybridoptimalBIC2)
```
