...
multResidualPlot <- function(residual.list, fitted.list) {
  ...
  for (i in 1:3) {
    x <- fitted.list[[i]] 
    y <- residual.list[[i]]
    col <- rep(colors[i], times=length(x))
    #, inherit.aes = FALSE)
    length(a)
    print(dim(a)[1])
    df.temp <- data.frame(x, y, col)
    colnames(df.temp) <- c("x", "y", "col")
    a <- rbind(a, df.temp)
    print(dim(a)[1])
    print("we got here")
  }
  
  mrp <- ggplot(a, aes(x=x, y=y, colour=col)) + geom_point(alpha=0.3, size=0.75) + geom_smooth()
  return(mrp)
}
...
multResidualPlot(residual.list, fitted.list)

i<-1
plot(y=residual.list[[i]], x=fitted.list[[i]] , col=colors[i])
